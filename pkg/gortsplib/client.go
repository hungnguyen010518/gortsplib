...
func AddCustomData(originBuf []byte, customData []byte) []byte {
	rtspLines := strings.Split(string(originBuf), "\r\n")
	separatorIndex := len(rtspLines) // Default to the end of the slice if no body is present
	for i, line := range rtspLines {
		if line == "" { // This is the separator line
			separatorIndex = i
			break
		}
	}
	s_customData := "\r\n" + string(customData)
	s_modifiedBuf := ""
	headerLines := rtspLines[:separatorIndex]
	if separatorIndex != len(rtspLines) {
		bodyLines := rtspLines[separatorIndex+1:]
		body := ""
		if len(bodyLines) > 0 {
			body = strings.Join(bodyLines, "\r\n")
		}
		headerLines = append(headerLines, s_customData)
		s_modifiedBuf = strings.Join(headerLines, "\r\n") + "\r\n\r\n" + body
	} else {
		headerLines = append(headerLines, string(customData))
		headerLines = append(headerLines, "")
		s_modifiedBuf = strings.Join(headerLines, "\r\n")
	}
	modifiedBuf := []byte(s_modifiedBuf)
	return modifiedBuf
}
...
type Client struct {
	...
	chSetParameter chan setParameterReq
	chPlay         chan playReq
...
func (c *Client) Start(scheme string, host string) error {
	// RTSP parameters
	if c.ReadTimeout == 0 {
		c.ReadTimeout = 5000 * time.Millisecond
	}
	if c.WriteTimeout == 0 {
		c.WriteTimeout = 5000 * time.Millisecond
	}
	if c.DialTimeout == 0 {
		c.DialTimeout = 500 * time.Millisecond
	}
	if c.InitialUDPReadTimeout == 0 {
		c.InitialUDPReadTimeout = 3000 * time.Millisecond
	}
	...
	c.chSetParameter = make(chan setParameterReq)
	...
	c.useGetParameter = true
	c.DisableRTCPSenderReports = true
...
func (c *Client) runInner() error {
	for {
		...
		case req := <-c.chSetParameter:
			res, err := c.doSetParameter(req.baseURL, req.crd)
			req.res <- clientRes{res: res, err: err}
...
func (c *Client) doClose() {
	...
	if c.nconn != nil && c.baseURL != nil {
		header := base.Header{
			"WG67-Version": base.HeaderValue{c.Wg67Version},
		}
...
func (c *Client) stopReadRoutinesByPause() {
	c.stopReadRoutines()
	c.keepaliveTimer = time.NewTimer(c.KeepAlivePeriod)
}
...
func (c *Client) connOpen() error {
	...
	dialCtx, dialCtxCancel := context.WithTimeout(c.ctx, c.DialTimeout)
	defer dialCtxCancel()
...
func (c *Client) doSetup(
	...
	case TransportTCP:
		...
		if c.UseInterleaved {
			th.UseInterleaved = true
		} else {
			th.UseInterleaved = false
		}
	...
	case TransportTCP:
		...
		// }
		if thRes.InterleavedIDs == nil {
			cm.tcpChannel = th.InterleavedIDs[0]
		} else {
			if (thRes.InterleavedIDs[0] + 1) != thRes.InterleavedIDs[1] {
				return nil, liberrors.ErrClientTransportHeaderInvalidInterleavedIDs{}
			}
...
func (c *Client) doSetParameter(u *base.URL, crd []byte) (*base.Response, error) {
	err := c.checkState(map[clientState]struct{}{
		clientStateInitial:   {},
		clientStatePrePlay:   {},
		clientStatePreRecord: {},
	...
	res, err := c.do(&base.Request{
		...
		URL:    u,
		Header: base.Header{
			"Content-Type": base.HeaderValue{"application/x-crd+xml"},
			"WG67-Version": base.HeaderValue{c.Wg67Version},
		},
		Body: crd,
	}, false)
...
func (c *Client) SetParameter(u *base.URL, crd []byte) (*base.Response, error) {
	cres := make(chan clientRes)
	select {
	case c.chSetParameter <- setParameterReq{baseURL: u, crd: crd, res: cres}:
		res := <-cres
		return res.res, res.err
...
func (c *Client) doRecord(crd []byte) (*base.Response, error) {
	err := c.checkState(map[clientState]struct{}{
		clientStatePreRecord: {},
	})
	if err != nil {
...
func (c *Client) Record(crd []byte) (*base.Response, error) {
	cres := make(chan clientRes)
	select {
	case c.chRecord <- recordReq{res: cres, crd: crd}:
		res := <-cres
		return res.res, res.err
...
func (c *Client) doPause(crd []byte) (*base.Response, error) {
	err := c.checkState(map[clientState]struct{}{
		clientStatePlay:   {},
		clientStateRecord: {},
	})
...
func (c *Client) Pause(crd []byte) (*base.Response, error) {
	cres := make(chan clientRes)
	select {
	case c.chPause <- pauseReq{res: cres, crd: crd}:
		res := <-cres
		return res.res, res.err
