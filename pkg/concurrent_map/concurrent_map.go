...
func create[K comparable, V any](sharding func(key K) uint32) ConcurrentMap[K, V] {
	m := ConcurrentMap[K, V]{
		...
		outerMutexs: make([]*sync.Mutex, SHARD_COUNT),
	}
	for i := 0; i < SHARD_COUNT; i++ {
		m.shards[i] = &ConcurrentMapShared[K, V]{items: make(map[K]V)}
		m.outerMutexs[i] = &sync.Mutex{}
	}
	return m
}
...
func (m *ConcurrentMap[K, V]) OuterLock(key K) {
	m.outerMutexs[m.GetShardIndex(key)].Lock()
}

func (m *ConcurrentMap[K, V]) OuterUnLock(key K) {
	m.outerMutexs[m.GetShardIndex(key)].Unlock()
}
